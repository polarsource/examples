...
export default function HomePage() {
  ...
  // This function is now updated to call your internal API route
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-pink-50 dark:from-gray-900 dark:via-black dark:to-pink-950/40 text-gray-800 dark:text-gray-200">
      <div className="flex flex-col items-center justify-center p-6 pt-12">
        ...
        <h1 className="text-5xl font-extrabold mt-4 text-center text-gray-900 dark:text-white">Get Your Tokens</h1>
        ...
        <div className="mt-8">
          {customerPortalUrl ? (
            <Button size="lg" onClick={() => router.replace(customerPortalUrl)}>Go to Customer Portal</Button>
          ) : (
            <Button size="lg" onClick={fetchCustomerPortal}>Create Customer Portal</Button>
          )}
        </div>
        ...
        <div className="w-full max-w-5xl mt-16">
          {loading ? (
            ...
            <div className="space-y-8">
              {items.map((p: any) => (
                <div key={p.id} className="bg-white/60 dark:bg-gray-900/70 backdrop-blur-sm rounded-xl shadow-lg p-6 grid md:grid-cols-2 gap-6 items-center">
                  <div>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{p.name}</h3>
                  </div>
                  <div className="text-center md:text-right">
                    <Button onClick={() => buyToken(p.id)} size="lg">Buy Now</Button>
                  </div>
                </div>
              ))}
            </div>
