...
export default function HomePage() {
  ...
  const startCheckout = async () => {
    ...
    if (data.url) {
      window.location.href = data.url // redirect user to checkout page
    } else {
  ...
  const openCustomerPortal = async () => {
    ...
    if (data.url) {
      window.location.href = data.url // redirect user to portal
    } else {
    ...
    <main className="flex flex-col items-center justify-center min-h-screen gap-6">
      ...
      <div>
        <a
          ...
          href="https://buy.polar.sh/polar_cl_0EyuXVWFsYtYqIb7E1TPmP5g96zh139tlqJGW3DHVLv"
          data-polar-checkout
        ...
        {/* <button onClick={startCheckout} disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded-lg">
        {loading ? 'Loading...' : 'Start Checkout'}
        </button> */}

        <button onClick={openCustomerPortal} className="px-4 py-2 bg-green-600 text-white rounded-lg">
