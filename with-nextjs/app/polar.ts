...
export const polar = new Polar({
  accessToken: (): any => {
    ...
    if (!token) {
      throw new Error(`❌ ${process.env.NEXT_PUBLIC_POLAR_MODE === 'production' ? 'NEXT_PUBLIC_PRODUCTION_POLAR_ACCESS_TOKEN' : 'NEXT_PUBLIC_SANDBOX_POLAR_ACCESS_TOKEN'} token not found.`)
    }
